...
System \eqref{ode} can also seen as Liouville equation itself, since one may express $\mu_t$ via the pushforward $\mu_t = (\varphi^t(0,\cdot))_\ast \mu_0$, at least locally, where the flow $\varphi^t(0,\cdot)$ exists.
